...
class StockingSpec extends Specification with TestSupport {
  ...
  "Stocking" should {
    "add items with employee info in updating" in new WithApplication {
      ...
      updated.items.get.size must equalTo(newStocking.items.get.size)
    ...
    "add items with employee info in creating" in new WithApplication {
      val employeeId: Long = 998
      val employeeName: String = "employeeName"

      val exists: List[Stocking] = Stocking.index(93740362, 1, None, None, Some(1))
      val newStocking: Stocking = exists.head.copy(id = None, items = Some(exists.head.items.get ::: List(StockingDetail(None, exists.head.id, Some(1), None, Some(employeeId), Some(employeeName), None, None, None, None, None, None, None))))

      val created: Stocking = newStocking.create(93740362, 1).get

      created.items.get.size must equalTo(newStocking.items.get.size)

      val DataFromDb: Stocking = Stocking.show(93740362, 1, created.id.get).get
      DataFromDb.items.get.size must equalTo(created.items.get.size)
      DataFromDb.items.get.last.employee_id must beSome(employeeId)
      DataFromDb.items.get.last.employee_name must beSome(employeeName)
    }
