...
import models.V4.AgentRawActivity
import models.{ErrorResponse, SuccessResponse}
import play.api.libs.json.{JsError, Json}
import play.api.mvc.Controller

object AgentRawActivityController extends Controller with Secured {
  def show(agentId: Long, id: Long) = IsAgentLoggedIn {
    u => _ =>
      AgentRawActivity.show(id, agentId) match {
        case Some(x) =>
          Ok(Json.toJson(x))
        case None =>
  ...
  def index(agentId: Long, from: Option[Long], to: Option[Long], most: Option[Int]) = IsAgentLoggedIn { u => _ =>
    Ok(Json.toJson(AgentRawActivity.index(agentId, from, to, most)))
  }

  def create(agentId: Long) = IsAgentLoggedIn(parse.json) { u => request =>
    request.body.validate[AgentRawActivity].map {
  ...
  def delete(agentId: Long, id: Long) = IsAgentLoggedIn { u => _ =>
    AgentRawActivity.deleteById(id, agentId)
