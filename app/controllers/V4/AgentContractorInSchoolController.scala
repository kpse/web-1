...
import models.V4.{AgentContractorInSchool, AgentContractorInSchool$, AgentContractor}
import models.{ErrorResponse, SuccessResponse}
import play.api.libs.json.{JsError, Json}
import play.api.mvc.Controller

object AgentContractorInSchoolController extends Controller with Secured {
  def show(agentId: Long, kg: Long, id: Long) = IsAgentLoggedIn {
    u => _ =>
      AgentContractorInSchool.show(id, agentId, kg) match {
        case Some(x) =>
          Ok(Json.toJson(x))
        case None =>
  ...
  def index(agentId: Long, kg: Long, from: Option[Long], to: Option[Long], most: Option[Int]) = IsAgentLoggedIn { u => _ =>
    Ok(Json.toJson(AgentContractorInSchool.index(agentId, kg, from, to, most)))
  }

  def create(agentId: Long, kg: Long) = IsAgentLoggedIn(parse.json) { u => request =>
    request.body.validate[AgentContractorInSchool].map {
  ...
  def delete(agentId: Long, kg: Long, id: Long) = IsAgentLoggedIn { u => _ =>
    AgentContractorInSchool.deleteById(id, agentId, kg)
    Ok(Json.toJson(new SuccessResponse()))
  }

  def published(kg: Long, from: Option[Long], to: Option[Long], most: Option[Int]) = IsAgentLoggedIn {
    u => _ =>
      Ok(Json.toJson(AgentContractorInSchool.published(kg, from, to, most)))
  }
