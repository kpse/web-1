    ...
    ($scope, $rootScope, $stateParams, $location, School, Class, Parent) ->
      ...
      $scope.kindergarten = School.get school_id: $stateParams.kindergarten, ->
        $scope.kindergarten.classes = Class.bind({school_id: $stateParams.kindergarten}).query ->
          $location.path($location.path() + '/class/' + $scope.kindergarten.classes[0].class_id + '/list') if $location.path().indexOf("/class/",
            0) < 0
...
.controller 'ParentsInClassCtrl',
  ['$scope', '$rootScope', 'parentService', '$stateParams', '$location', '$http', 'uploadService', '$timeout',
   'schoolService', 'classService', '$alert',
    ($scope, $rootScope, Parent, $stateParams, $location, $http, uploadService, $timeout, School, Class, Alert) ->
      $scope.loading = true
