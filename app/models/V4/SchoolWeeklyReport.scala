...
case class SchoolWeeklyReport(id: Long, school_id: Long, week_start: String, logged_once: Long, logged_ever: Long, created_at: Long, child_count: Long, parent_count: Long) {
  def save = DB.withConnection {
    implicit c =>
      val time = System.currentTimeMillis
      Logger.debug(s"save school $school_id weekly statistics at $time")
      SQL(s"insert into school_weekly_statistics (school_id, week_start, child_count, parent_count, logged_once, logged_ever, created_at) values " +
        s"({school_id}, {week}, {child}, {parent}, {once}, {ever}, {time})")
        .on(
          'school_id -> school_id,
          'week -> week_start,
          'child -> child_count,
...
object SchoolWeeklyReport {
  ...
  def weeklyStats(agentId: Long)(implicit c: Connection) = {
      val start = dayPattern.print(DateTime.now().minusWeeks(4).withDayOfWeek(1))
      val end = dayPattern.print(DateTime.now().minusWeeks(1).withDayOfWeek(1))
  ...
  def weeklyCountingLogic(schoolId: Long, weekStart: DateTime): SchoolWeeklyReport = {
    ...
    val value: SchoolWeeklyReport = digFromCache[SchoolWeeklyReport](cacheKey, 3600 * 24 * 5, () => {
      weeklyHistoryData(schoolId, weekStart).getOrElse {
        val weekStartTimeStamp = weekStart.withDayOfWeek(1).withMillisOfDay(1)
        val weekEndTimeStamp = weekStart.plusWeeks(1).withDayOfWeek(1).withMillisOfDay(1)
        val data = SchoolOperationReport.collectInGivenTimePeriod(schoolId, weekStartTimeStamp, weekEndTimeStamp)
        val weekly = data.toWeekly(startDateString)
        weekly.save
        weekly
      }
    })
  ...
  val simple = {
    get[Long]("uid") ~
      ...
      case id ~ school ~ week ~ once ~ ever ~ child ~ parent ~ created =>
        SchoolWeeklyReport(id, school.toLong, week, once, ever, created, child, parent)
    }
  }
