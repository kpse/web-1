...
case class AgentAd(id: Option[Long], agent_id: Long, title: String, address: Option[String], contact: String, time_span: Option[String],
  ...
  def preview(agentId: Long) = DB.withConnection {
    implicit c =>
      SQL("update agentadvertisement set publish_status=99, updated_at={time} where uid={id} and agent_id={base} and publish_status in (0, 3)")
        .on(
          'id -> id,
          'base -> agentId,
  ...
  def publish(agentId: Long) = DB.withConnection {
    implicit c =>
      SQL("update agentadvertisement set publish_status=2, published_at={time}, updated_at={time} where uid={id} and agent_id={base} and publish_status in (99, 3)")
        .on(
          'id -> id,
          'base -> agentId,
          'time -> System.currentTimeMillis()
